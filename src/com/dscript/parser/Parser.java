...
public final class Parser {
	...
	private Statement statement() {
		...
		if(match(TokenType.OUTLINE)) {
			return new OutlineStatement(expression());
		}
		...
		if(match(TokenType.USES)) {
			return new UsesStatement(expression());
		}
	...
	private Statement assignementStatement() {
		...
		throw new RuntimeException("Error: unknown statement!");
	}
	...
	private Statement forStatement() {
		...
		final Statement initialization = assignementStatement();
		consume(TokenType.DOTCOMMA);
		final Expression termination = expression();
		consume(TokenType.DOTCOMMA);
		final Statement increment = assignementStatement();
	...
	private Token consume(TokenType type) {
		final Token current = get(0);
		if(type != current.getType()) throw new RuntimeException("Error: token " + current + "does not match " + type + "!");
		pos++;
		return current;
	}
